...
\begin{proof}  \bigbreak
  ...
  sepal length and petal width are similar across all three kinds of flower, and that perhaps most of these support vectors lie within the marginal hyperplanes (and so have non-zero slack variables). As $C$ is high, we can be confident that nevertheless they must deviate only by a little from the marginal hyperplanes.
  \bigbreak
