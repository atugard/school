...
\begin{question}[3 points]
  ...
  \begin{proof}
    ...
    \bigbreak
    As for the slack variables case. It turns out that at least for $C=5$ the algorithm will pick the hyperplane $x=0$. That such an answer exists makes sense,
    since for $x=0$, $x_1, x_3$ are correctly classified, and so we can put $\xi_1, \xi_3 = 0$. We can also set $\xi_2 = 1.8$, which would yield
    $y_i(w \cdot x_i + b) = -0.80  \geq 1-\xi_2 = -0.80$. Thus, at a first glance, it's at least reasonable to suspect such a hyperplane may exist.
